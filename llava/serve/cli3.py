...
def get_rag_prompt(query: str, rag_python_path="/home/aimgroup/anaconda3/envs/rag_faiss/bin/python", script_name="rag_fetcher.py"):
...
def main(args):
    ...
    used_pts = False  # ✅ 추가: Point Cloud 사용 여부 플래그

    def run_prompt(inp):
        nonlocal pts_tensor, used_pts  # ✅ 바깥 스코프 접근

        rag_augmented_input = get_rag_prompt(inp)
        print(f"{roles[1]}: ", end="")

        if pts_tensor is not None and not used_pts:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt-file", type=str, help="Optional prompt file to use instead of manual input.")
