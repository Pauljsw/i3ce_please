...
class MaskTransformer(nn.Module):
    ...
    def inference(self, pts):
        # print("📦 [DEBUG] ReCon.inference() called")
        recon_device = next(self.parameters()).device
        # print(f" - pts.device: {pts.device}")
        # print(f" - self.device (ReCon): {recon_device}")

        with torch.no_grad():
            neighborhood, center = self.group_divider(pts)
            # print(f" - neighborhood.device (before): {neighborhood.device}")
            # print(f" - center.device (before): {center.device}")

            # ✅ 명시적 디바이스 통일
            neighborhood = neighborhood.to(recon_device)
            center = center.to(recon_device)

            # print(f" - neighborhood.device (after): {neighborhood.device}")
            # print(f" - center.device (after): {center.device}")

            # ✅ Embed + 디바이스 확인
            group_input_tokens = self.embed(neighborhood)
            # print(f" - group_input_tokens.device (after embed): {group_input_tokens.device}")

            batch_size, seq_len, C = group_input_tokens.size()

            # ✅ global_query 디바이스 통일
            global_query = self.global_query.expand(batch_size, -1, -1).to(group_input_tokens.device)
            # print(f" - global_query.device: {global_query.device}")

            # ✅ pos_embed 디바이스 강제 이동
            self.pos_embed = self.pos_embed.to(group_input_tokens.device)
            center = center.to(group_input_tokens.dtype).to(group_input_tokens.device)
            ...
            # ✅ Encoder 디바이스 통일
            self.encoder = self.encoder.to(group_input_tokens.device)
