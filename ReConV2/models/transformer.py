...
class PatchEmbedding(nn.Module):  # Embedding module
    ...
    def forward(self, point_groups):
        # print("🔥 [DEBUG] PatchEmbedding.forward called")
        # print(f" - point_groups.shape: {point_groups.shape}")
        # print(f" - point_groups.device (before any op): {point_groups.device}")

        # 모든 first_conv layer들의 device 확인
        for i, layer in enumerate(self.first_conv):
            if hasattr(layer, 'weight'):
                # print(f"📌 first_conv[{i}].weight.device: {layer.weight.device}")
                # 🔁 device mismatch 발생 가능성 차단을 위해 to()
                layer.to(point_groups.device)

        # self.second_conv도 같이 확인 및 이동
        for i, layer in enumerate(self.second_conv):
            if hasattr(layer, 'weight'):
                # print(f"📌 second_conv[{i}].weight.device: {layer.weight.device}")
                layer.to(point_groups.device)

        conv_device = next(self.first_conv.parameters()).device
        # print(f" - conv_device: {conv_device}")

        point_groups = point_groups.to(conv_device, non_blocking=True)
        # print(f" - point_groups.device (after to()): {point_groups.device}")

        bs, g, n, c = point_groups.shape
        point_groups = point_groups.reshape(bs * g, n, c).transpose(1, 2)
        # print(f" - point_groups.device (final): {point_groups.device}")
